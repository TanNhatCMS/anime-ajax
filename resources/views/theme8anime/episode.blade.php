                        ...
                        <ul class="list-episode">
                            <li class="episode">
                                @foreach ($currentMovie->episodes->where('slug', $episode->slug)->where('server', $episode->server) as $server)
                                    <a data-id="{{ $server->id }}" data-link="{{    episode_link( $server->link, $server->type) }}"
                                        data-type="{{episode_type($server->type)}}" onclick="chooseStreamingServer(this)"
...
@push('scripts')
    <script src="/themes/8anime/player/js/p2p-media-loader-core.min.js"></script>
    <script src="/themes/8anime/player/js/p2p-media-loader-hlsjs.min.js"></script>
            ...
            if (type == 'embed') {
                if (vastAds) {
                    ...
                    const objSetupFake = {
                        ...
                        width: "100%",
                        height: "100%",
                        file: "/themes/8anime/player/1s_blank.mp4",
                        volume: 100,
                        mute: false,
                        autostart: true,
                        advertising: {
                    ...
                    fake_player.on('complete', function(event) {
                        ...
                        wrapper.innerHTML = `<iframe width="100%" height="100%" class="lazyload" data-src="${link}" frameborder="0" scrolling="no"
                    allowfullscreen="" allow='autoplay' seamless="" ></iframe>`
                        fake_player.remove();
                    });
    ...
    <script type="text/javascript" src="/themes/8anime/js/watch.js"></script>
