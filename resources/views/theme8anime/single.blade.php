    ...
    <main>
        ...
        <article class="TPost Single">
            <header>
                ...
                <div class="Image">
                    <figure class="Objf">
                        @includeWhen(filter_var($currentMovie->thumb_url, FILTER_VALIDATE_URL) ,'themes::theme8anime.inc.html.img',  [
                            'width' => 180,
                            'height' => 260,
                            ...
                            'title' => $currentMovie->name.' - '.$currentMovie->origin_name,
                        ])
                        @includeUnless(filter_var($currentMovie->thumb_url, FILTER_VALIDATE_URL) ,'themes::theme8anime.inc.html.picture',  [
                            'width' => 180,
                            'height' => 260,
                            'src' => $currentMovie->thumb_url,
                            'alt' => $currentMovie->name.' - '.$currentMovie->origin_name,
            ...
            <footer class="ClFx">
                <div class="VotesCn">
                    ...
                    <div style="display: none;" itemprop="aggregateRating" itemscope
                        ...
                        @php
                            $currentMovie->rating_star 

                        @endphp
                        @if($currentMovie->rating_count)
                        <span itemprop="ratingValue">{{ number_format($currentMovie->rating_star , 1) }}</span>
                        <meta itemprop="ratingCount" content="{{ $currentMovie->rating_count }}">
                        @else
            ...
            @if ($currentMovie->poster_url)
                <div class="TPostBg Objf">
                    @includeWhen(filter_var($currentMovie->poster_url, FILTER_VALIDATE_URL) ,'themes::theme8anime.inc.html.img',  [
                       'src' => $currentMovie->poster_url,
                        'alt' => $currentMovie->name.' - '.$currentMovie->origin_name,
                        'title' => $currentMovie->name.' - '.$currentMovie->origin_name,
                        'class' => 'TPostBg',
                    ])
                    @includeUnless(filter_var($currentMovie->poster_url, FILTER_VALIDATE_URL) ,'themes::theme8anime.inc.html.picture',  [
                        'src' => $currentMovie->poster_url ?: $currentMovie->thumb_url,
                        'alt' => $currentMovie->name.' - '.$currentMovie->origin_name,
                        'title' => 'Hình ảnh '. $currentMovie->name,
                        'class' => 'TPostBg',
                        'slug' => $currentMovie->slug,
                    ])
                </div>
        ...
        <div class="MovieInfo TPost Single">
                ...
                <div class="MvTbCn anmt" id="MvTb-Cast">
                    <ul class="ListCast Rows AF A06 B03 C02 D20 E02">
                        {!! $currentMovie->actors->map(function ($actor) {
                            if(isset($actor->thumb_url)){
                                return '<li><a href="' .$actor->getUrl() .'" title="Diễn viên ' .$actor->name .'">
                                    <figure> 
                                        <span class="Objf">
                                            <img src="'.$actor->thumb_url.'" alt="Diễn viên ' . $actor->name .'">
                                        </span>
                                ...
                                return '<li><a href="' .$actor->getUrl() .'" title="Diễn viên ' .$actor->name .'">
                                    <figure> 
                                        <span class="Objf">
                                            <img src="/themes/8anime/images/cast-image.png" alt="Diễn viên ' . $actor->name .'">
                                        </span>
                                        <figcaption>' .$actor->name .'</figcaption>
                                    </figure></a></li>';
                            }
            ...
            @endif
            @php
                $trailer_id = getYoutubeIdFromUrl($currentMovie->trailer_url);
            @endphp
            @if ($trailer_id)
                        ...
                        <div class="TPlayer">
                            <div class="TPlayerTb Current clearfix" id="Opt1">
                                ...
                                <iframe width="560" height="315"
                                    class="lazyload" data-src="https://www.youtube.com/embed/{{ $trailer_id }}" allowfullscreen></iframe>
                            </div>
            ...
            <div class="MvTbCn anmt" id="MvTb-Image">
                <div class="ImageMovieList owl-carousel">
                    <div class="item active">
                        @if ($currentMovie->poster_url)
                            <center>
                                @includeWhen(filter_var($currentMovie->poster_url ?: $currentMovie->thumb_url, FILTER_VALIDATE_URL) ,'themes::theme8anime.inc.html.img',  [
                                    'src' => $currentMovie->poster_url ?: $currentMovie->thumb_url,
                                    'alt' => 'Hình ảnh '. $currentMovie->name,
                                    'title' => 'Hình ảnh '. $currentMovie->name,
                                    'class' => 'img-responsive',
                                ])
                                @includeUnless(filter_var($currentMovie->poster_url ?: $currentMovie->thumb_url, FILTER_VALIDATE_URL) ,'themes::theme8anime.inc.html.picture',  [
                                    'src' => $currentMovie->poster_url ?: $currentMovie->thumb_url,
                                    'alt' => 'Hình ảnh '. $currentMovie->name,
                                    'title' => 'Hình ảnh '. $currentMovie->name,
                                    'class' => 'img-responsive',
        ...
        @if(get_theme_option('ads_on_comment'))
            {!! get_theme_option('ads_on_comment') !!}
        @endif
        <div class="Wdgt" id="comments">
            <div class="Title">Bình luận</div>
            <div class="Comment Wrt" id="respond">
                <div class="fb-comments  fb_iframe_widget" style="width: 100%; background-color: #fff" data-href="{{ $currentMovie->getUrl() }}" data-width="100%"
...
@push('scripts')
    ...
    <script type="text/javascript" src="/themes/8anime/js/film.notiny.js"></script>
    <script type="text/javascript" src="/themes/8anime/js/jquery.raty.js"></script>
    <script type="text/javascript" src="/themes/8anime/js/film.rating.js"></script>
@endpush
