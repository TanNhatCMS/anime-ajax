...
@section('catalog_filter')
    @include('themes::theme8anime.inc.catalog_filter')
@endsection
...
@section('content')
    <main>
            ...
            @if (count($data))
                <ul class="MovieList Rows AX A06 B04 C03 E20">
                    @foreach ($data ?? [] as $movie)
                        @include('themes::theme8anime.inc.section.section_thumb_item')
                    @endforeach
                </ul>
            @else
        ...
        {{ $data->appends(request()->all())->links('themes::theme8anime.inc.pagination') }}
    </main>
@endsection
