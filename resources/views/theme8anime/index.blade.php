...
@php
    ...
    $slider_recommended = Cache::remember('site.movies.slider_recommended', setting('site_cache_ttl', 5 * 60), function () {
        $lists = preg_split('/[\n\r]+/', get_theme_option('slider_recommended'));
        $data = [];
        foreach ($lists as $list) {
            if (trim($list)) {
                $list = explode('|', $list);
...
@section('slider_recommended')
    @include('themes::theme8anime.inc.slider_recommended')
    @foreach ($slider_recommended as $item)
