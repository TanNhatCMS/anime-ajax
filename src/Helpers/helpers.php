...
if (!function_exists('theme_url')) {
    function theme_url($src){
        return get_theme_option('home_url').$src;
    }
}
if (!function_exists('episode_type')) {
    function episode_type($type){
        if($type=='server_player_1' || $type=='server_player_2' || $type=='server_player_3' || $type=='server_player_4' || $type=='server_player_5'){
            return 'embed';
        }
        return $type;
    }
}
if (!function_exists('episode_link')) {

    function episode_link($link, $type) {
        if($type=='server_player_1' || $type=='server_player_2' || $type=='server_player_3' || $type=='server_player_4' || $type=='server_player_5'){
            return Setting::get($type).$link;
        }
        return $link;
    }
}